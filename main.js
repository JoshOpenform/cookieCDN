class Banner {
  ...
  initialize() {
    ...
    const isStrict = this.getCookie("Strictly") == "true" ? "checked" : "false";
    const isPerformance =
      this.getCookie("Performance") == "true" ? "checked" : "false";
    const isMarketing =
      this.getCookie("Marketing") == "true" ? "checked" : "false";
    const isFunctional =
      this.getCookie("Functional") == "true" ? "checked" : "false";
    const isAnalytical =
      this.getCookie("Analytics") == "true" ? "checked" : "false";
  ...
  categorizeCookies(data) {
    ...
    try {
      this.pendingCookies.forEach((cookie) => {
        ...
        let cookieEntry = data[`${cookieName}`];
        if (cookieEntry) {
  ...
  createEventListeners() {
    ...
    for (let i = 0; i < this.acceptButtons.length; i++) {
      this.acceptButtons[i].addEventListener("click", (e) => {
        const dataIndex = e.target.getAttribute("data-all");
        this.handleConsent(dataIndex);
      });
    }
  ...
  handleConsent(dataIndex = 0) {
    ...
    this.setPrefCookie("all", "true", 1, dataIndex);
    this.hideElement(this.bannerContainer);
    this.hideElement(this.settingsMenu);
  ...
  handleRejection() {
    ...
    // this.pendingCookies = [];
    //this.setPrefCookie("all","false",1);
    this.list = [];
    this.hideElement(this.bannerContainer);
    this.hideElement(this.settingsMenu);
  }
  ...
  setPrefCookie(cookieName, agreed, list = false) {
    if (!list) {
      this.setCookie(cookieName, `${agreed}`, 7);
      return;
    }
    this.setCookie("Strictly", agreed, 7);
    this.setCookie("Performance", agreed, 7);
    this.setCookie("Analytics", agreed, 7);
    this.setCookie("Marketing", agreed, 7);
    this.setCookie("Functional", agreed, 7);
  }
  ...
  updatePreference() {
    ...
    this.setPrefCookie("Strictly", sNCToggle.checked);
    this.setPrefCookie("Performance", pCToggle.checked);
    this.setPrefCookie("Analytics", aCToggle.checked);
    this.setPrefCookie("Marketing", mCToggle.checked);
    this.setPrefCookie("Functional", fCToggle.checked);
...
function parseCSV(csvData) {
  ...
  rows.forEach((row) => {
    ...
    // Loop through headers and add data to cookieObj
    headers.forEach((header, index) => {
      try {
        cookieObj[header.trim()] = rowData[index].trim();
      } catch (error) {}
    });

    // Use cookie name as key for parsedData
    parsedData[cookieObj["Cookie / Data Key name"]] = cookieObj;
  });
